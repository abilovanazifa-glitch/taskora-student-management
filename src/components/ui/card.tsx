...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "group/card bg-card text-card-foreground flex flex-col gap-(--card-spacing) overflow-hidden rounded-3xl border border-border/70 py-(--card-spacing) text-base [--card-spacing:--spacing(6)] has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:[--card-spacing:--spacing(5)] data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-3xl *:[img:last-child]:rounded-b-3xl",
        className,
      )}
      {...props}
    />
...
function CardDescription({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("text-muted-foreground text-body-sm leading-6", className)}
      {...props}
    />
  );
}
